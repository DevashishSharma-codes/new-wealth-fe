...
export function FloatingDropdownModal({
  ...
  if (isFullScreen) {
    const fullScreenJSX = (
      <div
        className="fixed inset-0 z-[9999] w-screen h-screen bg-[#FAF7F2] flex flex-col overflow-hidden select-none animate-fade-in text-[#2B2A28]"
        aria-modal="true"
        role="dialog"
      >
        {/* Fixed Header */}
        <div className="shrink-0 h-16 sm:h-20 px-6 sm:px-10 border-b border-[#EFE9DF] bg-[#FAF7F2] flex items-center justify-between gap-4 z-10">
          <div>
            <h3 className="font-heading text-base sm:text-xl font-extrabold text-[#2B2A28] flex items-center gap-2">
              {title}
            </h3>
        ...
        {/* Search Bar */}
        {showSearch && (
          <div className="shrink-0 px-6 sm:px-10 py-4 bg-[#F5F0E6]/50 border-b border-[#EFE9DF]">
            <div className="max-w-4xl mx-auto relative flex items-center">
              <SearchIcon className="absolute left-4 w-5 h-5 text-[#F0883E] pointer-events-none" />
              <input
                ref={searchInputRef}
                type="text"
                value={searchQuery}
                ...
                className="neu-field w-full pl-12 pr-10 py-3 text-sm font-semibold rounded-2xl outline-none transition-all"
              />
              {searchQuery && (
                <button
                  type="button"
                  onClick={() => setSearchQuery('')}
                  className="absolute right-4 text-xs font-bold text-[#8A8578] hover:text-[#F0883E] cursor-pointer"
                >
        ...
        <div className="flex-1 overflow-y-auto p-6 sm:p-10 scrollbar-thin bg-[#FAF7F2]">
          <div className="max-w-7xl mx-auto">
            {filteredOptions.length === 0 ? (
              <div className="py-20 text-center text-sm text-[#8A8578] italic font-medium">
                {emptyMessage}
              </div>
            ) : (
                    ...
                    <button
                      ...
                      className={`w-full text-left p-4 rounded-2xl border transition-all duration-150 flex items-start justify-between gap-3 ${
                        option.disabled
                          ...
                          : 'border-[#EFE9DF] bg-white/70 hover:bg-[#FFF6ED]/80 hover:border-[#F0883E]/50 hover:shadow-md text-[#2B2A28] cursor-pointer'
                      }`}
                    >
                      <div className="flex items-start gap-3 min-w-0 flex-1">
                        {option.icon && typeof option.icon !== 'string' ? (
                          <div className="shrink-0 w-9 h-9 rounded-xl bg-[#F1EDE6] border border-[#EFE9DF] flex items-center justify-center text-[#2B2A28] shadow-inner mt-0.5">
                            {option.icon}
                          </div>
                        ) : (
                          <div className="shrink-0 w-9 h-9 rounded-xl bg-[#FFF6ED] border border-[#EFE9DF] flex items-center justify-center text-[#F0883E] mt-0.5">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {option.subtext && (
                            <div className="text-xs font-medium text-[#8A8578] truncate mt-0.5">
                              {option.subtext}
                            </div>
                          )}
                        </div>
                      ...
                      <div className="flex items-center gap-2 shrink-0 text-right">
                        ...
                        {isSelected && (
                          <span className="w-6 h-6 rounded-full bg-[#F0883E] text-white flex items-center justify-center text-xs font-black shadow-sm">
        ...
        {/* Fixed Footer */}
        <div className="shrink-0 h-16 sm:h-20 px-6 sm:px-10 border-t border-[#EFE9DF] bg-[#FAF7F2] flex items-center justify-between gap-4 text-xs font-semibold text-[#8A8578] z-10">
  ...
  const normalJSX = (
    <div
      className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-[#1C1B1A]/40 backdrop-blur-md transition-all duration-200 animate-fade-in"
      onClick={onClose}
      aria-modal="true"
      role="dialog"
    >
      <div
        className={`bg-[#FAF7F2] border border-[#EFE9DF] rounded-[28px] w-full ${widthClass} max-h-[65vh] flex flex-col p-5 sm:p-7 relative text-[#2B2A28] animate-popup-scale overflow-hidden select-none`}
        ...
        {showSearch && (
          <div className="py-3 shrink-0">
            <div className="relative flex items-center">
              <SearchIcon className="absolute left-3.5 w-4 h-4 text-[#F0883E] pointer-events-none" />
              <input
                ref={searchInputRef}
                type="text"
                value={searchQuery}
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    {option.icon && typeof option.icon !== 'string' ? (
                      <div className="shrink-0 w-7 h-7 rounded-xl bg-[#F1EDE6] border border-[#EFE9DF] flex items-center justify-center text-[#2B2A28] shadow-inner">
                        {option.icon}
                      </div>
                    ) : (
                      <div className="shrink-0 w-7 h-7 rounded-xl bg-[#FFF6ED] border border-[#EFE9DF] flex items-center justify-center text-[#F0883E]">
                  ...
                  <div className="flex items-center gap-2 shrink-0 text-right">
                    {option.rightTag && (
                      <span className={`text-xs font-bold px-2 py-0.5 rounded-xl border ${isSelected ? 'bg-[#FFF6ED] text-[#F0883E] border-[#F0883E]/30' : 'bg-[#EFE9DF]/60 text-[#8A8578] border-[#EFE9DF]'}`}>
                        {option.rightTag}
                      </span>
                    )}
                    {isSelected && (
